...
export function CriteriaCard({
  ...
  return (
    <div className="group relative">
      <div className="absolute -inset-0.5 bg-gradient-to-r from-[var(--chrome-bg-accent)] via-[#ffffff] to-[var(--chrome-border-glow)] rounded-xl opacity-75 group-hover:opacity-100 transition duration-300 blur-sm"></div>
      <div className="relative bg-gradient-to-br from-[var(--chrome-bg-surface)] to-[var(--chrome-bg-hover)] rounded-xl border border-white/50 p-4 shadow-[0_4px_16px_rgba(0,0,0,0.06)]">
        <div className="flex items-start justify-between mb-3">
          <div className="flex items-center gap-2">
            <h3 className="text-base font-medium bg-gradient-to-b from-[var(--chrome-text-heading)] to-[var(--chrome-text-body)] bg-clip-text text-transparent">
              {getCriterionLabel(criterion)}
            </h3>
            <span className="text-xs px-2.5 py-0.5 bg-gradient-to-b from-[var(--chrome-bg-muted)] to-[var(--chrome-bg-inset)] text-[var(--chrome-text-secondary)] rounded-full border border-white/60 shadow-[inset_0_1px_0_rgba(255,255,255,0.8)]">
              {getCriterionTypeLabel(criterion.type)}
            </span>
          </div>
          <button
            onClick={handleRemove}
            className="p-1.5 text-[var(--chrome-text-disabled)] hover:text-red-500 hover:bg-red-50 rounded-lg transition-all duration-200"
